from fastapi import APIRouter

from app.services.cbf_service import simulate_cbf, simulate_cbf_comparison

router = APIRouter(prefix="/cbf", tags=["cbf"])


@router.get("/simulate")
def cbf_simulate(
    steps: int = 150,
    dt: float = 1.0,
    seed: int = 42,
    alpha: float = 0.5,
    cbf_enabled: bool = True,
):
    return simulate_cbf(steps=steps, dt=dt, seed=seed, alpha=alpha, cbf_enabled=cbf_enabled)


@router.get("/compare")
def cbf_compare(
    steps: int = 150,
    dt: float = 1.0,
    seed: int = 42,
    alpha: float = 0.5,
):
    return simulate_cbf_comparison(steps=steps, dt=dt, seed=seed, alpha=alpha)


@router.get("/multi_seed")
def cbf_multi_seed(
    steps: int = 150,
    dt: float = 1.0,
    alpha: float = 0.5,
):
    seeds = [0, 1, 7, 13, 21, 42, 99, 123]
    results = []
    for seed in seeds:
        r = simulate_cbf(steps=steps, dt=dt, seed=seed, alpha=alpha, cbf_enabled=True)
        results.append({
            "seed": seed,
            "min_M": r["min_M"],
            "safety_violated": r["safety_violated"],
            "time_below_safe": r["time_below_safe"],
            "avg_recovery_time": r["avg_recovery_time"],
        })
    all_safe = all(not r["safety_violated"] for r in results)
    return {"results": results, "all_seeds_safe": all_safe}
