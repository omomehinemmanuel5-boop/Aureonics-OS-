...
def estimate_decay_lambda(similarities: list[float], time_deltas: list[float]) -> float:
    ...
    # CCP paper framing: coherence decay c(t) ~ exp(-lambda * delta_t)
...
def compute_ccp(anchor_context: str, responses: list[str], time_deltas: list[float] | None = None) -> dict[str, float]:
    if not responses:
        return {"ccp": 0.0, "lambda": 0.0, "mean_similarity": 0.0}
    ...
    # Higher CCP means high coherence with low decay.
    ccp = clamp01(mean_similarity / (1.0 + decay_lambda))
    return {
        "ccp": round(ccp, 4),
...
def entropy_proxy(text: str) -> float:
    # Placeholder base probability from lexical diversity.
    tokens = tokenize(text)
    if not tokens:
        return 0.0
...
def compute_iec(pairs: list[tuple[str, str]]) -> dict[str, float]:
    if not pairs:
        return {"iec": 0.0, "variance": 1.0, "mean_ratio": 0.0}
...
def compute_adv(decisions: list[str], compliance_flags: list[bool]) -> dict[str, float]:
    if not decisions:
        return {"adv": 0.0, "variance": 0.0, "compliance": 0.0}
    ...
    adv = clamp01(variance * compliance)
    return {
        "adv": round(adv, 4),
...
def compute_profile(tasks: list[Any], projects: list[Any] | None = None) -> dict[str, float]:
    ...
    for t in tasks:
        if t.from_signal:
            ...
            output_text = f"{t.title} {t.task_type or ''} {t.mode or ''}"
